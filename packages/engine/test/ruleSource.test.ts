 ...
 *
 * **PAY-004 joined it and then left the rule set entirely.** It changed hands when the corpus was
 * re-based (D-140), because the requirement was real and only its attribution was wrong; it was
 * removed when the requirement turned out to be post-boarding and therefore not a screening question
 * at all (D-142). CATG-007 is the sole Mintro-authored rule again.
 *
 * The assertion below **names the set rather than counting it**, which is why it caught both moves.
 * A rule quietly acquiring Mintro authorship, and a rule quietly losing it, are the same defect seen
 * from either side — and a count would have passed on the wrong pair.
 */
  ...
  it('everything quoting the published standards says so, and the Mintro observations are named', () => {
    ...
    */
    ...
    expect([...mintro].sort()).toEqual([
      ...
      'PROD-013',
      'PROD-014',
      // D-259's two. Neither quotes the standards — the published document says nothing about a
      // promised measurable result or about lifestyle association — so both are Mintro's own
