 ...
 *
 * **Where** is therefore the page the row's capture is of, classified by the vertical's page-type
 * table — not the surfaces the rule declares, which live in the rule set and not in the snapshot. For
 * a rule that reads a region of a page, such as the footer, that resolves to the page holding it,
 * which is the page the capture shows. A manual rule has no page at all, and says so by saying
 * nothing.
 */
